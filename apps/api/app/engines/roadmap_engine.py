...
ITEMS: list[dict[str, Any]] = [
    ...
    item("user_key_boost", "User Key Boost", "extension", "IMPLEMENTED", "Bring-your-own LLM key per session, held in an encrypted TTL vault (Redis in distributed deployments) and cleared on logout.", 100, [], "v2.5", "Security Guild", "HIGH", "BETA", "MEDIUM", ["ia", "security", "backend"], apis=["POST /api/user-ai-keys/session", "DELETE /api/user-ai-keys/session"], contracts=["user-key-boost.contract.ts"], docs=["Security notes"], risk_basis=["Secrets must never be logged or persisted.", "Tests verify validation errors do not echo raw keys."]),
