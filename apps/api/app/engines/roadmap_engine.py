...
ITEMS: list[dict[str, Any]] = [
    ...
    item("user_key_boost", "User Key Boost", "extension", "IMPLEMENTED", "Named bring-your-own LLM keys encrypted at rest, owner-scoped, and gated by persisted provider validation.", 100, [], "v2.5", "Security Guild", "HIGH", "BETA", "MEDIUM", ["ia", "security", "backend"], apis=["POST /api/user-ai-keys", "POST /api/user-ai-keys/{key_id}/test", "DELETE /api/user-ai-keys/{key_id}"], contracts=["ai-key-vault.contract.ts"], docs=["docs/architecture/user-key-boost.md"], risk_basis=["Raw secrets must never be logged or returned.", "Tests verify encryption, tenant isolation, and redacted validation errors."]),
