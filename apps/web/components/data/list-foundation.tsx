...
const statusLabel: Record<FoundationStatus, string> = {
  ready: 'Ready',
  planned: 'Planned',
  blocked: 'Blocked',
  draft: 'Draft',
...
export function FilterBar({ children }: { readonly children?: ReactNode }) {
  return (
    <Card className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap gap-2">{children ?? <Badge>Foundation only</Badge>}</div>
    </Card>
  );
}
  ...
  return (
    <div className="micro-interaction flex items-center justify-between gap-4 rounded-[var(--radius-xl)] border border-[color:var(--border)] bg-white/5 p-4">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <Button type="button" variant="ghost" className="h-9 w-9 rounded-full p-0" aria-label={`Open actions for ${title}`}>
