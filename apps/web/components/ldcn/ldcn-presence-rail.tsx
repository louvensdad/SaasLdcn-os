      ...
      <div className="relative space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="ds-caption text-[color:var(--muted)]">{t('ldcn.context')}</p>
            <p className="mt-1 text-sm font-semibold text-[color:var(--text)]">{context.current_phase}</p>
            <p className="ds-caption mt-1 text-[color:var(--muted)]">{stepLabel}</p>
          </div>
        </div>
        ...
        <details className="advanced-disclosure rounded-[var(--radius-md)] border border-[color:var(--border)] bg-black/10">
          <summary className="focus-ring flex cursor-pointer items-center justify-between gap-3 rounded-[var(--radius-md)] px-3 py-2">
            <span className="ds-caption text-[color:var(--muted)]">{t('ldcn.contextDetails')}</span>
          ...
          <div className="rounded-[var(--radius-xl)] border border-white/10 bg-white/[0.04] p-3">
            <p className="ds-caption text-[color:var(--muted)]">{t('ldcn.route')}</p>
          ...
          <div className="rounded-[var(--radius-xl)] border border-white/10 bg-black/10 p-3">
            <p className="ds-caption text-[color:var(--muted)]">{t('ldcn.pipeline')}</p>
        ...
        {actions?.length ? (
          <div className="space-y-2">
            <p className="ds-caption text-[color:var(--muted)]">{t('ldcn.reservedFutureActions')}</p>
            <div className="grid gap-2">
              {actions.map((action) => (
                <Badge key={action} className="justify-start border-white/10 px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--muted)]">
                  {action.replaceAll('_', ' ')}
                </Badge>
              ))}
            </div>
