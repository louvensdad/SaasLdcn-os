...
export function TeamTopologySurface({ topology }: { readonly topology: TeamTopology }) {
  return (
    <Card className="grid gap-4 p-5" data-testid="team-topology-surface">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.28em] text-[color:var(--muted)]">Team Topology</p><p className="mt-2 text-sm text-[color:var(--muted)]">{topology.coordination}</p></div><UsersRound className="h-5 w-5 text-[color:var(--success)]" /></div>
      ...
      <Badge className="w-fit border-white/10 bg-white/[0.04]">Required maturity: {topology.maturity}</Badge>
    </Card>
  );
}
