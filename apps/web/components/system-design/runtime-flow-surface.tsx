...
export function RuntimeFlowSurface({ flow }: { readonly flow: RuntimeFlow }) {
  ...
  return (
    <Card className="grid gap-4 p-5" data-testid="runtime-flow-surface">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.28em] text-[color:var(--muted)]">{t('systemDesign.runtime.title')}</p><p className="mt-2 text-sm text-[color:var(--muted)]">{t('systemDesign.runtime.description')}</p></div><Workflow className="h-5 w-5 text-[color:var(--accent)]" /></div>
