...
export function RiskZonesSurface({ zones }: { readonly zones: readonly RiskZone[] }) {
  return (
    <Card className="grid gap-4 p-5" data-testid="risk-zones-surface">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.28em] text-[color:var(--muted)]">Risk Visualization</p><p className="mt-2 text-sm text-[color:var(--muted)]">Complexity, security, scaling and operational risk zones.</p></div><AlertTriangle className="h-5 w-5 text-[color:var(--warning)]" /></div>
