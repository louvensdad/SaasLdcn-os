...
export function DependencyGraphSurface({ visualization }: { readonly visualization: DependencyVisualization }) {
  ...
  return (
    <Card className="grid gap-4 p-5" data-testid="dependency-visualization-surface">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.28em] text-[color:var(--muted)]">{t('systemDesign.dependencies.title')}</p><p className="mt-2 text-sm text-[color:var(--muted)]">{t('systemDesign.dependencies.description')}</p></div><GitBranch className="h-5 w-5 text-[color:var(--accent)]" /></div>
