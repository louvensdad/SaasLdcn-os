...
async function openVisualizationReview(page: Page, request: APIRequestContext) {
  ...
  // The cockpit is tabbed (only the active tab's content is mounted).
  await page.getByRole('tab', { name: 'Architecture Cockpit' }).click();
}

test('wizard shows the architecture cockpit and topology updates', async ({ page, request }) => {
  await openVisualizationReview(page, request);
...
test('wizard runtime, risk and readiness visualization are visible', async ({ page, request }) => {
  await openVisualizationReview(page, request);
...
test('wizard visualization stays responsive on mobile', async ({ page, request }) => {
...
test('wizard visualization exposes safe state when backend is offline', async ({ page, request }) => {
  await page.route('**/api/system-design/**', async (route) => route.abort('failed'));
  await openVisualizationReview(page, request);
